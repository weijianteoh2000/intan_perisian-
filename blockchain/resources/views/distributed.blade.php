...
@section('script')
    ...
    <script>
        ...
        function createAPeerBlock() {
            ...
            if (totalBlock >= 100) {
                showError("Not allowed to create more block")
                return;
            }
        ...
        $('#containerDiv').on('click', '.deploy_btn', function() {
            ...
            let data = $(`.peer_${peer_id} #block${count}_data`).val();
            if (!data) {
                let id = $(`.peer_${peer_id} #block${count}_data`).data("id")
                checkData(peer_id, id);
                return;
            }

            deploy(peer_id, count);
        ...
        function deploy(peer_id, count) {
            ...
            $(`.peer_${peer_id} .step_${count} textarea`).val(comment);

            if (count == stepCount) {
                $(`.peer_${peer_id} .step_${count}`).show();
                $(`.peer_${peer_id} .add_more_block`).show();
                $(`.peer_${peer_id} .lightImg_${count}`).hide();
                $(`.peer_${peer_id} .greenImg_${count}`).show();
                $(`.peer_${peer_id} #block${count}`).removeClass('bgLight');
                $(`.peer_${peer_id} #block${count}`).addClass('bg-lightGreen');
            }
            if (count != stepCount) {
                $(`.peer_${peer_id} #block${count}`).removeClass('bgLightRed').addClass('bg-lightGreen');
                $(`.peer_${peer_id} .redChainImg${(count - 1)}`).hide();
                $(`.peer_${peer_id} .redImg_${count}`).hide();
            ...
            if (count >= 2) {
                $(`.peer_${peer_id} .lightGreenChainImg${(count - 1)}`).show();
                $(`.peer_${peer_id} .redChainImg${(count - 1)}`).hide();
            }
        ...
        function addMoreBlock(peerCount, count, data) {
            ...
            //  Create Hash
            stepCount = count;
            var addedBlockCount = $(`.peer_${peerCount}`).length;
            if (addedBlockCount == count) {
                stepCount++;
            ...
            hashData(data, nonce)
                .then(hashedResult => {
                    ...
                    if (TOTAL_PEER > 1) {
                        duplcateNewBlock(CURRENT_PEER, stepCount, data, hash);
                    }
                    /* if (TOTAL_PEER <= 1) {
                        $(`.peer_${CURRENT_PEER} #block${stepCount}_previous_hash`).text(hash);
                    } else {
        ...
        function duplcateNewBlock(peer_id, count, data, hash) {
            var newBlockIndex = count;
            var clonedElement = $(`#containerDiv .peer${peer_id}_block${count}`).clone();

            for (let i = 1; i <= TOTAL_PEER; i++) {
                if (peer_id != i) {
                    // var newClass = 'peer_' + next;
                    var newClass = 'peer_' + i;
                    clonedElement.removeClass('peer_' + peer_id).addClass(newClass);
                    var newClass2 = `peer${i}_block${count}`;
                    clonedElement.removeClass(`peer${peer_id}_block${count}`).addClass(newClass2);

                    clonedElement.find('textarea').attr('data-peer_id', i);

                    clonedElement.find('.deploy_btn').attr('data-peer_id', i);

                    var index = count - 1;
                    $(`#containerDiv .peer${i}_block${(index)}`).after(clonedElement);
                    $(`.${newClass2}`).css('display', 'none');
                    deploy(i, index);
                    $(`.peer_${i} #block${index}_data`).val(data);
                    $(`.peer${i}_block${index} #block${index}_hash`).text(hash);
                }
            }
        }
